...
namespace SS
{
    ...
    public partial class SpreadsheetPanel : UserControl
    {
        ...
        public Dictionary<int, int> SelectedCells; 
        ...
        private class DrawingPanel : Panel
        {
            ...
            private void MousePosition(out int x, out int y)
            {
            ...
            protected override void OnPaint(PaintEventArgs e)
            {
                ...
                //Highlights the cell the mouse cursor is over                           
                int xpos, ypos; 
                MousePosition(out xpos, out ypos);
                int startX = ((xpos - (LABEL_COL_WIDTH)) / DATA_COL_WIDTH) * DATA_COL_WIDTH + LABEL_COL_WIDTH; 
                int startY = ((ypos - (LABEL_ROW_HEIGHT)) / DATA_ROW_HEIGHT) * DATA_ROW_HEIGHT + LABEL_ROW_HEIGHT; 
                
                if (xpos > LABEL_COL_WIDTH && ypos > LABEL_ROW_HEIGHT && xpos < (COL_COUNT * DATA_COL_WIDTH))
                {
                    e.Graphics.FillRectangle(new SolidBrush(Color.FromArgb(50, Color.SteelBlue)), (startX), 
                        (startY), DATA_COL_WIDTH, DATA_ROW_HEIGHT);
                }
                ...
                if ((_selectedCol - _firstColumn >= 0) && (_selectedRow - _firstRow >= 0))
                {
                    ...
                    e.Graphics.FillRectangle(new SolidBrush(Color.FromArgb(100, Color.SteelBlue)), 
