	...
	std::vector<Matrix4f>& p_global)
{
	...
	{
		int parent = p_jointParent[j]; 

		//make sure alignment is right
		if (parent < j)
		{
			//if joint is zero / the root joint
			if (j == 0)
			{
			...
			{
				p_global[j] = p_global[parent] * p_offset[j] * p_local[j];
			}
		}
	}
}
		...
		std::vector<Vector3f>& p_deformedVertices)
{
	for (unsigned int v = 0; v < p_vertices.size(); v++)
	{
		...
		Vector4f homog_p_vertices = toHomog(p_vertices[v]); 
		Vector4f homog_p_deformedVertices; 
		homog_p_deformedVertices.setZero(); 

		for (int j = 0; j < p_numJoints; j++)
		{
			homog_p_deformedVertices += p_weights[j][v] * p_jointTrans[j] * p_jointTransRestInv[j] * homog_p_vertices; 
		}

		p_deformedVertices[v] = fromHomog(homog_p_deformedVertices); 
