...
void DCT(float* x, const float* y, const float* q, int size)
{
	...
	for (int i = 0; i < size; i++)
	{
		x[i] = dotProduct(q + (i*8), y, 8); 
	}
}

void InverseDCT(float* y, const float* x, const float* q, int size)
{
	// takes a vector x and produce as output a vector y where y_i = sum x_k * q_k_i
...
void compressWAVSignal()
{
	...
	for (int i = 0; i < g_wav_size; i += 8) //g_wav_size
	{
